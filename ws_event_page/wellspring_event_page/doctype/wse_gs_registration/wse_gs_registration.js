...
frappe.ui.form.on("WSE GS Registration", {
  refresh(frm) {
    ...
    if (frm.doc.status === "Waiting for Approval") {
      // Add Approve Registration button
      let approveBtn = frm.add_custom_button("Approve", function () {
        frappe.confirm(
          "Bạn có chắc muốn phê duyệt đăng ký này? | Are you sure you want to approve this registration?",
          function () {
            frappe.call({
              method:
                "ws_event_page.api.event.greatest_show.registration.approve_registration",
      ...
      approveBtn.addClass("btn-primary");
        ...
        function () {
          frappe.confirm(
            ...
            function () {
              frappe.call({
                method:
                  "ws_event_page.api.event.greatest_show.registration.resend_confirmation_email",
                args: { registration_id: frm.doc.name },
                callback: function (r) {
                  if (r.message) {
                    frappe.msgprint(r.message.message);
      ...
      let cancelBtn = frm.add_custom_button("Cancel", function () {
        frappe.confirm(
          "Bạn có chắc muốn huỷ đăng ký này? | Are you sure you want to cancel this registration?",
          function () {
            frappe.call({
              method:
                "ws_event_page.api.event.greatest_show.registration.cancel_registration",
              args: { registration_id: frm.doc.name },
              callback: function (r) {
                if (r.message) {
                  frappe.msgprint(r.message.message);
