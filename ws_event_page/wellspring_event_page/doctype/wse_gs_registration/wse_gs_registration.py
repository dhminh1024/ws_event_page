...
class WSEGSRegistration(Document):
	...
	if TYPE_CHECKING:
		...
		status: DF.Literal["Cancelled", "Approved", "Waitting"]
	...
	def after_insert(self):
		...
		if self.status == GSRegistrationStatus.WAITTING.value:
	...
	def send_confirmation_email(self):
		...
		sender = settings.email_sender or frappe.utils.get_formatted_email("Administrator")
		...
		program_title = None
		if self.gs_program:
			program_title = frappe.db.get_value("WSE GS Program", self.gs_program, "title_en")
