...
def get_certificate(certificate_token):
    ...
    try:
        ...
        if not certificate_token:
            return {
                "success": False,
                "message": _("Certificate token is required")
            }
        ...
        if not registration_name:
            return {
                "success": False,
                "message": _("Invalid certificate token. Please check your email for the correct link.")
            }
        ...
        if not registration.certificate_generated:
            return {
                "success": False,
                "message": _("Certificate is not yet available. Please check back later or contact the school.")
            }
        ...
        return {
            "success": False,
            "message": _("Failed to retrieve certificate. Please try again or contact support.")
        }
...
def resend_certificate_email(certificate_token, parent_email):
    ...
    try:
        ...
        if not registration_name:
            return {
                "success": False,
                "message": _("Invalid certificate token")
            }
        ...
        try:
            ...
            return {
                "success": True,
                "message": _("Certificate email has been resent to {0}").format(parent_email)
            }

        except Exception as email_error:
            frappe.log_error(
                message=str(email_error),
                title="KDD Resend Certificate Email Failed"
            )
            return {
                "success": False,
        ...
        return {
            "success": False,
            "message": _("Failed to resend certificate email. Please try again or contact support.")
        }
