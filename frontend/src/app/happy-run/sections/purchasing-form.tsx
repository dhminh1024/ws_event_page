...
export const PurchasingForm: FC<PurchasingFormProps> = ({ className }) => {
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    ...
    const hr_tickets_count = tickets.reduce((sum,i)=> sum + (i.ticket_type === "Happy Run" ? 1 : 0), 0);
    const wb_tickets_count = tickets.reduce((sum,i)=> sum + (i.ticket_type === "Well-being" ? 1 : 0), 0);

    if (hr_tickets_count > 0 && Number(event.variables.enable_happy_run_ticket?.value ?? 0) === 0) {
      alert("Happy Run ticket is not available");
      return;
    }
    if (wb_tickets_count > 0 && Number(event.variables.enable_well_being_ticket?.value ?? 0) === 0) {
                            ...
                            >
                              ...
                              <SelectContent className="max-h-none bg-white">
                                {ticket_classes.map((item) => {
                                  if(item.value === "Well-being" && Number(event.variables?.enable_well_being_ticket?.value ?? 1) === 0) return null;
                                  if(item.value === "Happy Run" && Number(event.variables?.enable_happy_run_ticket?.value ?? 1) === 0) return null;
                                  return (
                                    <SelectItem
                                      key={item.value}
